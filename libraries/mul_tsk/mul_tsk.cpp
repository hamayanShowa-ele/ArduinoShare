  ...
  VP_INT exinf1, VP_INT exinf2, VP_INT exinf3, VP_INT exinf4 )
{
  ...
  if( tid < 0 || tid >= vMAX_TASK_NUMBER ) return E_ID;

  // Initialize task stack
  tcb[tid].sp = (void *)((char *)stk + stk_sz - (4 * (9 + 2)));
    // Register r4 to register r12 and lr register are stored in this area
  ptr = (VP_INT *)tcb[tid].sp;
  ptr[9] = (VP_INT)tsk;  /*pc*/
  // from ptr[0]:R4 to ptr[8]:R12
...
ER rot_rdq( void )
{
  ...
  for( i = 0; i < vMAX_TASK_NUMBER; i++ )
  {
    if( ++tid >= vMAX_TASK_NUMBER ) tid = 0;  // Switch to next task
...
//    while( (RELTIM)(systim - lastTim) < dly ) {}  // Waiting for time to elapse
    while( (RELTIM)(millis() - lastTim) < dly ) {}  // Waiting for time to elapse
    return E_OK;
  }
...
//    if( (systim - lastTim) >= dly )  // Waiting for time to elapse
    if( (millis() - lastTim) >= dly )  // Waiting for time to elapse
    {
...
ER rel_wai( ID tid )
{
  if( tid < 0 || tid >= vMAX_TASK_NUMBER ) return E_ID;     // Incorrect ID
...
{
  if( tid < 0 || tid >= vMAX_TASK_NUMBER ) return E_NOEXS;
